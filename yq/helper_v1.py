"""
The base is copied from utils/mask_dictionary_model.py
Yongming
"""
...
class MaskDictionaryModel:
    ...
    """
    Compare the masks till now and the freshly masks from Ground Dino and SAM2 Image predictor
    tracking_annotation_dict: the masks till now, MaskDictionaryModel
    fresh_masks: the freshly predicted masks, MaskDictionaryModel, which is this object
    """
    def compare_two_masks(self, tracking_annotation_dict, iou_threshold=0.8):
    ...
    """
    yq tracking_annotation_dict: the masks till now, MaskDictionaryModel, which seems only contain the annotation of the current frame calculated in last processing
    """
    def update_masks(self, tracking_annotation_dict, iou_threshold=0.8, objects_count=0):
        updated_masks = {}

        for seg_obj_id, seg_mask in self.labels.items():  # tracking_masks #yq freshly predicted masks
            flag = 0 
            ...
            if not flag:
                print(f"len(self.labels): {len(self.labels)}", f"len(tracking_annotation_dict.labels): {len(tracking_annotation_dict.labels)}", f"objects_count: {objects_count+1}")
